...
object Main extends App {
  ...
  private lazy val seed = if (ConfFromOptions.randomSeed.isSupplied)
    ConfFromOptions.randomSeed.toOption.get.toLong
  else
    ConfigFromFile.seed

  private lazy val tag: String = if (ConfFromOptions.tag.isSupplied)
    ConfFromOptions.tag.toOption.get
  else
    ConfigFromFile.tag.getOrElse("")

  private lazy val growthRate = if (ConfFromOptions.growthRate.isSupplied)
    ConfFromOptions.growthRate.get
  else
    ConfigFromFile.growthRate

  private val params = EventSimulatorParams(nUsers = nUsers,
